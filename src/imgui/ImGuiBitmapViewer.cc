...
void ImGuiBitmapViewer::paint(MSXMotherBoard* motherBoard)
{
	...
	im::Window(title.c_str(), &show, [&]{
		...
		// Viewer modes include planar/non-planar variants; command modes do not.
		int commandMode = (mode == SCR5) ? 0 : (mode == SCR6) ? 1
		                : (mode == SCR7 || mode == SCR7NP) ? 2 : 3;
		auto formatted = VDPCmdEngine::formatCommand(regs, commandMode, vdp->isECOM());
